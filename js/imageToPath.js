...
function correctColor([r, g, b]) {
  const br = (r + g + b) / 3;
  if (br > 220) return [255, 220, 120]; // near-white → golden
  if (br <  25) return [255, 160,  60]; // near-black → warm orange
  return [r, g, b];
}
...
function extractDominantColor(data) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    if (data[i+3] > 64) {
      ...
      if (pr < 200 || pg < 200 || pb < 200) { r += pr; g += pg; b += pb; count++; }
    }
  }
  if (count === 0) return [255, 200, 50];
...
// Keep one representative point per minSpacing×minSpacing grid cell
function gridDownsample(pts, minSpacing) {
  const grid = new Map();
  for (const [x, y] of pts) {
    const key = `${Math.floor(x/minSpacing)},${Math.floor(y/minSpacing)}`;
    if (!grid.has(key)) grid.set(key, [x, y]);
  }
...
// Engine's launch() does cy-(ny-0.5)*SCALE, so ny=0→top of display. Canvas y=0 is also top → no flip needed.
function normalizePoints(pts) {
  ...
  return pts.map(([x, y]) => [(x - minX + padX) / range, (y - minY + padY) / range]);
}
...
export async function imageUrlToPathSegments(url, size = 256) {
  ...
  if (boundary.length < 20) throw new Error(`Too few boundary pixels (${boundary.length})`);
  ...
  const downsampled = gridDownsample(boundary, 3);
  const ordered     = orderByAngle(downsampled);
  ...
  console.log('[imageToPath]', {
    ...
    afterGridDownsample: downsampled.length,
    sampledPoints:     normalized.length,
    pathCount:         1,
    dominantColor:     color,
    minX, maxX, minY, maxY,
  });
